...
locals {
  ...
  azs                = [for index in range(local.az_count) : data.aws_availability_zones.available.names[index]]
  ...
  private_subnets    = [for index in range(local.az_count) : cidrsubnet(local.private_cidr, 2, index)]
  public_subnets     = [for index in range(local.az_count) : cidrsubnet(local.public_cidr, 2, index)]
}
